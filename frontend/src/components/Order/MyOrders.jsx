...
const MyOrders = () => {
    ...
    useEffect(() => {
        ...
        const filtered = orders.filter(order => {
            ...
            if (orderTime !== 0) {
                if (orderTime === dt.getMonth()) {
                    timeMatch = orderDate.getMonth() === orderTime;
                } else {
                    timeMatch = orderDate.getFullYear() === orderTime;
                }
            }
    ...
    const searchOrders = (e) => {
        e.preventDefault();
        if (!search.trim()) {
            setFilteredOrders(orders); // Reset to all orders when empty
...
{orders && filteredOrders.map((order) => {
    ...
    return orderItems.map((item, index) => (
        // Add null check for product and spread properly
        item.product && (
            <OrderItem 
                key={`${_id}-${index}`} 
                {...item.product} 
...
{filteredOrders?.map((order) => {
    ...
    return orderItems.map((item, index) => (
        item.product && (
            <OrderItem 
                key={`${_id}-${item.product._id}-${index}`}
